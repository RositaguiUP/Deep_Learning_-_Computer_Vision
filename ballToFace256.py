...
def build_encoder_decoder(input_shape=(256, 256, 3)):
    ...
    x = layers.Reshape((8, 8, 512))(x)
        ...
        # Decoder
    x = layers.Conv2DTranspose(256, (3, 3), strides=2, activation='relu', padding='same')(x)  # 8 → 16
    x = layers.BatchNormalization()(x)

    x = layers.Conv2DTranspose(128, (3, 3), strides=2, activation='relu', padding='same')(x)  # 16 → 32
    x = layers.BatchNormalization()(x)

    x = layers.Conv2DTranspose(64, (3, 3), strides=2, activation='relu', padding='same')(x)   # 32 → 64
    x = layers.BatchNormalization()(x)

    x = layers.Conv2DTranspose(32, (3, 3), strides=2, activation='relu', padding='same')(x)   # 64 → 128
    x = layers.BatchNormalization()(x)

    x = layers.Conv2DTranspose(3, (3, 3), strides=2, activation='sigmoid', padding='same')(x) # 128 → 256
